...
if (-not (RunSQLScript -FilePath $schemaFile -Description "Database Schema")) {
    Write-Host "Schema application had issues, but continuing..." -ForegroundColor Yellow
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "NPM install had issues" -ForegroundColor Yellow
    }
    Pop-Location
}
catch {
    Write-Host "Failed to run npm install: $_" -ForegroundColor Yellow
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "Seed script had issues" -ForegroundColor Yellow
    }
    Pop-Location
}
catch {
    Write-Host "Failed to run seed script: $_" -ForegroundColor Yellow
}
...
"@
    ...
    if ($tables.Count -gt 0) {
        ...
        Write-Host "No tables found. Database may not be properly initialized." -ForegroundColor Yellow
    }
...
Write-Host "1. cd local-backend" -ForegroundColor Cyan
